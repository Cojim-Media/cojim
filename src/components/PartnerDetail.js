...
const PartnerDetail = ({ toggleEditModal, itemDetails, setItemDetails, isEditModalOpen }) => {
    ...
    const handleApprove = (e) => {
        ...
        (async () => {
            const rawResponse = await fetch('/api/admin/approve-partner', {
                method: 'PATCH',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    const handleDeleteItem = (_id) => {
        ...
        (async () => {
            const rawResponse = await fetch('/api/admin/delete-partner', {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ partnerId: itemDetails._id })
            });
            const content = await rawResponse.json();
            const status = rawResponse.status;
            // Redirect the user to login page if status == 401
                            ...
                            <div className="relative max-w-md mx-auto md:max-w-full break-words bg-white w-full mb-6 shadow-lg rounded-xl">
                                <div className="px-6">
                                    ...
                                    <div className="bg-white p-3 shadow-xl rounded-sm">
                                        ...
                                        <Link
                                            to={`../partner-payment-list/${itemDetails._id}`}
