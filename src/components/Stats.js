...
const Stats = () => {
    ...
    useEffect(() => {
        // send a get request to the server to fetch partners data
        (async () => {
            const rawResponse = await fetch(`/api/admin/partners-list`, {
                method: 'GET',
            });
            ...
            if (content.error) {
                ...
                (async () => {
                    ...
                    if (content.error) {
                        ...
                        let totalReviewedAccount = partners.filter((item) => item.reviewed === true);
                        setStats({
                            ...stats,
                            totalAccounts: partners.length,
                            totalReviewedAccount: totalReviewedAccount.length,
                            totalUnreviewedAccounts: partners.length - totalReviewedAccount.length,
