 ...
 */
  ...
  const handleMarkOrderAsDelivered = (e) => {
    e.preventDefault();
    setSubmitted(true);
    // send a patch request to the server to update memeber
    (async () => {
      const rawResponse = await fetch('/api/sales/mark-delivered', {
        method: 'PATCH',
        headers: {
  ...
  useEffect(() => {
    ...
    (async () => {
      const rawResponse = await fetch(`/api/sales/list-sales?startDate=${0}&endDate=${0}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
      ...
      <div className="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        {/* <div className="flex items-center justify-center py-4">
          <div className="relative">
            ...
            <input onChange={(e) => { setStartDate(e.target.value) }} value={startDate} name="startDate" type="date" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Select date start" />
          </div>
          ...
          <div className="relative">
            ...
            <input onChange={(e) => { setEndDate(e.target.value) }} value={endDate} name="endDate" type="date" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Select date end" />
          </div>
        </div> */}
          ...
          <div data-modal-show="true" aria-hidden="true" className={`${isOrderDetailsOpen ? 'flex' : 'hidden'} modal bg-overlay flex flex-col justify-start items-center fixed z-50 h-full w-full inset-0 visible opacity-100 transition-all-300 overflow-auto`}>
            <div className="flex justify-center my-10 w-full">
              <div className="scale-100 w-[900px] min-w-[250px] bg-gray-200 rounded-lg px-3 pb-3 pt-7 mx-3 md:m-5 relative">
                <button onClick={() => toggleOrderDetails(orderDetails)} className="absolute top-0 right-0 sm:text-white sm:bg-primary sm:hover:bg-blue-500 transition-all-300 sm:top-[-10px] sm:right-[-10px] sm:rounded-lg p-2">
                ...
                <div className="flex flex-col gap-5">
                          ...
                          return (
                            <span key={index} className="cursor-pointer bg-white hover:bg-gray-100 flex flex-col sm:flex-row justify-between items-center gap-5 w-full p-2" href="/#">
                              <div className="border rounded-lg h-[40px] w-[40px] min-w-[40px] overflow-hidden">
                                <img className="w-full h-full object-cover" src={`${item.productId.thumbnailFile ? item.productId.thumbnailFile.url : ''}`} alt="product" />
                              </div>
                  ...
                  <div className="bg-white rounded-lg p-2 sm:p-5">
                    <span className="block font-semibold pb-4">Billing Details</span>
                    <div className="flex flex-wrap justify-between items-center py-1">
                      <span>Address:</span>
                      <span>
                        {orderDetails.billingData.address}
                      </span>
                    </div>
                    <div className="flex flex-wrap justify-between items-center py-1">
                      <span>Full Name:</span>
                      <span>
                        {orderDetails.billingData.fullname}
                      </span>
                    </div>
                    <div className="flex flex-wrap justify-between items-center font-bold py-1">
                      <span>Email Address:</span>
                      <span>{orderDetails.billingData.phone}</span>
                    </div>
                    <div className="flex flex-wrap justify-between items-center font-bold py-1">
                      <span>Contact:</span>
                      <span>{orderDetails.billingData.email}</span>
                    </div>
                    {
                    ...
                    <div className="flex justify-center">
                      <button onClick={handleMarkOrderAsDelivered} className={`w-max bg-green-900 text-white uppercase font-bold rounded-lg p-2 px-3 m-2`} href="/#">
                        <span className="text-center">Mark as Delivered</span>
                      </button>
                    </div>
                  </div>
        ...
        {
          isOrderDetailsOpen &&
          <div modal-backdrop="" className="bg-gray-900 bg-opacity-50 fixed inset-0 z-40"></div>
        }
      </div>
    </div>
  )
